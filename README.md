...
1. `src/content/people/ogrenci-1.md` dosyasını kopyalayın, adını kişinin adıyla değiştirin (ör. `ayse-yilmaz.md`). Dosya adı sayfa adresi olur: `/ekip/ayse-yilmaz`.
...
`role` alanı kişinin hangi grupta görüneceğini belirler: `pi`, `postdoc`, `phd`, `msc`, `bsc`, `alumni` (mezun).
Bir öğrenci mezun olduğunda `role: alumni` ve `graduated: "2027 · Yüksek Lisans"` yazmanız yeterli.

### Yeni proje eklemek

`src/content/projects/ornek-proje.md` dosyasını kopyalayıp doldurun.
